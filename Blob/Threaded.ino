...
void initModem() {
  ...
  while (!connected) {
    connected = nbAccess.begin(PINNUMBER) == NB_READY;
    Serial.println("Failed to connect to the modem. Retries...");
  }
...
/*     Serial.print("   ✓\tAccessed SIM-card:\t\t");
    boolean sim_ready = nbAccess.begin() == NB_READY;
...
void testGET() {
  Serial.print("Connecting to HTTP GET client: ");
  Serial.print(SERVER);
  Serial.print(":");
  Serial.println(S_PORT);

  if (client.connect(SERVER, S_PORT)) {
    Serial.println("   ✓\tConnected to HTTP GET client");

    // Correctly formatted HTTP request
    client.print("GET / HTTP/1.1");  
    client.print("Host: ");
    client.println(SERVER);
    client.println("User-Agent: Arduino/1.0"); // Some servers require this
    client.println("Accept: */*"); // Accept any response type
    client.println("Connection: close"); // Ensure double CRLF to end request
  } else {
    Serial.println("   x\tHTTP GET connection failed");
  }
}
